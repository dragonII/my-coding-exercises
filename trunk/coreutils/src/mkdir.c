...
"), stdout);
  ...
  -p, --parents         no error if existing, make parent direcories as needed\n\
...
struct mkdir_options
{
    /* Function to make an ancestor, or NULL if ancestors should not be
       made. */
    int (*make_ancestor_function)(char*, char*, void*);
...
static void
announce_mkdir(char* dir, void* options)
{
    struct mkdir_options* o = options;
    if(o->create_directory_format)
        prog_fprintf(stdout, o->create_directory_format, quote(dir));
}
...
static int make_ancestor(char* dir, char* component, void* options)
{
    struct mkdir_options* o = options;
...
int main(int argc, char** argv)
{
    ...
    options.mode_bits = 0;
    options.create_directory_format = NULL;
    ...
    while((optc = getopt_long(argc, argv, "pm:vZ:", longopts, NULL)) != -1)
    {
        switch(optc)
        {
            ...
            case 'm':
                ...
                break;
            case 'v':   /* --verbose */
                options.create_directory_format = _("create directory %s");
                break;
            case 'Z':
                scontext = optarg;
                break;
