...
/* Prepend the name S of lenght S_LEN, to the growing file_name, P */
static void
...
static void robust_getcwd(struct file_name* file_name)
{
    ...
    struct dev_ino* root_dev_info = get_root_dev_ino(&dev_ino_buf);
    struct stat dot_sb;

    if(root_dev_info == NULL)
    ...
    while(1)
    {
        ...
        if(SAME_INODE(dot_sb, *root_dev_info))
            break;
