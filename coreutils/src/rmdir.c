...
/* For long options that have no equivalent short option, use a
   non-character as a pseduo short option, starting with CHAR_MAX + 1. */
enum
{
...
static bool
ignorable_failure(int error_number, char* dir)
{
    ...
    while(1)
    {
        ...
        if(!ok)
        {
            ...
            else
            {
                ...
                error(0, errno, _("failed to remove directory %s"), quote(dir));
            }
            break;
        }
    }
