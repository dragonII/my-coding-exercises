...
#define AUTHORS "Jim Meyering & Eric Blake"
...
static size_t
count_consecutive_X_s(char* s, size_t len)
{
    size_t n = 0;
    ...
    if(use_dest_dir)
    {
        ...
        else
        {
            ...
            if(IS_ABSOLUTE_FILE_NAME(template))
                error(EXIT_FAILURE, 0,
                        _("invalid template, %s; with --tmpdir, it may not be absolute"),
                        quote(template));
        }
