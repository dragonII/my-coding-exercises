...
/* For long options that have no equivalent short option, use a
   non-character as a pseudo short option, start with CHAR_MAX + 1. */
enum
{
...
static gid_t parse_group(char* name)
{
    gid_t gid = -1;
    if(*name)
    {
...
"), stdout);
        fputs(_("\
      --no-preserve-root    do not treat `/' specically (the default)\n\
...
\n\
  ...
  -L                        traverse every symbolic link to a directory\n\
                            encountered\n\
  -P                        do not traverse any symbolic link (default)\n\
\n\
"), stdout);
...
int main(int argc, char** argv)
{
    ...
    /* Bit flags that control how fit works */
