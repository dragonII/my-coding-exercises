...
static void writeline(struct linebuffer *line, FILE *stream, int class)
{
    switch(class)
    {
        case 1:
        ...
        case 2:
            if(!only_file_2)
                return;
            /* Print a delimeter if we are printing lines from file 1 */
            if(only_file_1)
 ...
 */
static void check_order(struct linebuffer *prev,
                        struct linebuffer *current,
                        int whatfile)
{
