...
/* Group the digits of NUMBER according to the grouping rules of the
   ...
   each byte inserted. Returned the starting address of the modified
   number.
...
group_number(char* number, size_t numberlen,
             char* grouping, char* thousands_sep)
{
    register char* d;
...
/* Convert N to a human readable format in BUF, using the options OPTS.
   ...
   automatically; TO_BLOCK_SIZE must be 1 but is has no effect on the
   output. Use powers of 1024 if (OPTS & haman_base_1024), and powers
   ...
   If (OPTS & haman_space_before_unit), use a space to separate the
   number from any suffix that is append as described below.

   If (OPTS & human_SI), append an SI suffix indicating which power is
