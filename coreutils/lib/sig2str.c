...
static struct numname { int num; char name[8]; } numname_table[] =
{
    /* Signals required by POSIX 1003.1-2001 base, listed in
       traiditional numeric order */
#ifdef SIGHUP
    NUMNAME (HUP),
#endif
#ifdef SIGINT
...
static int str2signum(char* signame)
{
    if(ISDIGIT(*signame))
    {
        char* endp;
    ...
    else
    {
                ...
                return numname_table[i].num;
        {
            ...
            if(rtmin > 0 && strncmp(signame, "RTMIN", 5) == 0)
            {
                long int n = strtol(signame + 5, &endp, 10);
                if(! *endp && n >= 0 && rtmax - rtmin >= n)
                    return rtmin + n;
            }
            else if(rtmax > 0 && strncmp(signame, "RTMAX", 5) == 0)
            {
                long int n = strtol(signame + 5, &endp, 10);
